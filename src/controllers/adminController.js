...
const recharge = async(req, res) => {
    ...
    const [recharge] = await connection.query('SELECT * FROM recharge WHERE status = 0  AND type !="Manual" ');
    const [rechargeManual] = await connection.query('SELECT * FROM recharge WHERE status = 0 AND type="Manual"');
    ...
    const [withdrawCrypto] = await connection.query('SELECT * FROM withdraw WHERE status = 0 AND walletType !="INR"');
    return res.status(200).json({
        message: 'Success',
        status: true,
